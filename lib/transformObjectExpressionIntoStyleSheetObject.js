...
function canEvaluate(expr, context) {
  if (_babelCore.types.isLiteral(expr)) {
    ...
    return context.hasOwnProperty(expr.callee.object.name) && _typeof(context[expr.callee.object.name]) === "object" && context[expr.callee.object.name].hasOwnProperty(expr.callee.property.name) && typeof context[expr.callee.object.name][expr.callee.property.name] === "function" && expr.arguments.reduce(function (result, expr) {
